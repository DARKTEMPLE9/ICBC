 ...
 */
@Slf4j
public class DefaultRequestExchange implements RequestExchange {

    private String sysCode;

    public DefaultRequestExchange(String sysCode) {
    ...
    public JSONObject exchangeRequest(JSONObject reqObj) {
        ...
        String[] toArcFlags = toArcFlag.split(CmsConstants.TO_ARC_FLAG_SEPARATOR);
        Map<String, Object> flagMap = new HashMap<>();
        for (String flag : toArcFlags) {
            String[] flagArr = flag.split(CmsConstants.KEY_VALUE_SEPARATOR);
        ...
        if (toArc) {
            if (null != reqObj.get(CmsConstants.FILE_LIST_KEY)) {
                ...
                for (int i = 0; i < fileArray.size(); i++) {
                    JSONObject fileObj = fileArray.getJSONObject(i);
                    fileObj.put(CmsConstants.TO_ARC_KEY, CmsConstants.TO_ARC_VALUE_YES);
                }
            }
        }
