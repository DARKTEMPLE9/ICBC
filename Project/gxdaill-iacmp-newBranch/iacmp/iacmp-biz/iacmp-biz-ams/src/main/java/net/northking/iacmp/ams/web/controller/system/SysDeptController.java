...
public class SysDeptController extends BaseController {
    ...
    public AjaxResult editSave(@Validated SysDept dept) {
        ...
        } else if (dept.getParentDeptId().equals(dept.getDeptId())) {
     ...
     * 加载副部门（数据权限）列表树
     */
    @GetMapping("/userDeptTreeData")
    @ResponseBody
    public List<Ztree> deptUserTreeData(SysUser user) {
        List<Ztree> ztrees = deptService.userDeptTreeData(user);
        return ztrees;
    }
     ...
     * 加载档案盒所属部门列表树
     */
    @GetMapping("/boxDeptTreeData")
    @ResponseBody
    public List<Ztree> boxDeptTreeData() {
        ...
        roles.remove(23L);
        Long roleId = Collections.max(roles);
        List<Ztree> ztrees = new ArrayList<>();
        if (roleId == 4L) {//行档案管理员，查询全部部门
            ztrees = deptService.selectManageDept(sysUser);
        } else {//部门档案管理员,查询所属部门及辅部门
            ztrees = deptService.selectManageDept(sysUser);
        }
        return ztrees;
    }
