...
@Controller
@RequestMapping("/pdf/ams/fileView")
public class FileViewController {
    ...
    public void OfficeView(@PathVariable("fileImageId") String fileImageId, HttpServletRequest request, HttpServletResponse response) throws Exception {
        IWebOffice officeServer = new IWebOffice(imFileService, fileImageId);
    ...
    public AjaxResult fileCheckEncryption(@PathVariable("fileImageId") String fileImageId) {
        ...
        File file = null;
        try {
            ImFile imFile = imFileService.selectImFileById(fileImageId);
            boolean b = UploadUtil.downloadTrans(SysConfigInitParamsUtils.getConfig(CmsConstants.SERVER_PATH), imFile.getFileName(), imFile.getFilePath() + "&hadoopType=" + imFile.getHadoopType());
            if (!b) {
                throw new BusinessException("文件预览失败，服务器忙！");
            }

            file = new File(SysConfigInitParamsUtils.getConfig(CmsConstants.SERVER_PATH) + imFile.getFileName());
    ...
    public String openExcelView(String id, ModelMap modelMap) {
        modelMap.put("fileImageId", id);
        return "viewfile/fileExcel.html";
    }
...
class IWebOffice {
    ...
    public IWebOffice(IImFileService imFileService, String fileImageId) {
